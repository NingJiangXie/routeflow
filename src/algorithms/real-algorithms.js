...
class RealHCFA {
    ...
    getClusters() {
        // 简单的网格分区
        const clusters = [];
        const rowsPerCluster = Math.ceil(this.rows / this.nClusters);
        const colsPerCluster = Math.ceil(this.cols / this.nClusters);
        
        for (let cr = 0; cr < this.nClusters; cr++) {
            for (let cc = 0; cc < this.nClusters; cc++) {
                clusters.push({
                    rowMin: cr * rowsPerCluster,
                    rowMax: Math.min((cr + 1) * rowsPerCluster, this.rows),
                    colMin: cc * colsPerCluster,
                    colMax: Math.min((cc + 1) * colsPerCluster, this.cols)
                });
            }
        }
    ...
    plan(grid, start, goal) {
        ...
        // 每层的信息素
        let pheromoneLayers = clusters.map(() => 
        ...
        for (let iter = 0; iter < this.maxIterations; iter++) {
            ...
            for (let ant = 0; ant < this.nAnts; ant++) {
                ...
                while (!(current[0] === s_goal[0] && current[1] === s_goal[1]) && iterations < maxIter) {
                    ...
                    for (let i = 0; i < clusters.length; i++) {
                        const c = clusters[i];
                        if (current[0] >= c.rowMin && current[0] < c.rowMax &&
                            current[1] >= c.colMin && current[1] < c.colMax) {
                            currentLayer = i;
                            break;
                        }
                    }
                    ...
                    const probs = neighbors.map(n => {
                        ...
                        for (let i = 0; i < clusters.length; i++) {
                            const c = clusters[i];
                            if (n[0] >= c.rowMin && n[0] < c.rowMax &&
                                n[1] >= c.colMin && n[1] < c.colMax) {
                                nLayer = i;
                                break;
                            }
                        }
                        
                        const currentPheromone = pheromoneLayers[currentLayer][n[0]][n[1]];
                        const targetPheromone = pheromoneLayers[nLayer][n[0]][n[1]];
                        const avgPheromone = (currentPheromone + targetPheromone) / 2;
                        const distance = this.heuristic(n, s_goal);
                        
                        const prob = Math.pow(avgPheromone, this.alpha) * 
                                   Math.pow(1 / (distance + 0.1), this.beta);
                        totalProb += prob;
                        return prob;
                    });
            ...
            for (let l = 0; l < clusters.length; l++) {
                pheromoneLayers[l] = pheromoneLayers[l].map(row => 
                    row.map(val => val * (1 - this.rho))
                );
            }
            ...
            for (const path of allPaths) {
                for (const node of path) {
                    for (let l = 0; l < clusters.length; l++) {
                        const c = clusters[l];
                        if (node[0] >= c.rowMin && node[0] < c.rowMax &&
                            node[1] >= c.colMin && node[1] < c.colMax) {
                            pheromoneLayers[l][node[0]][node[1]] += this.Q / path.length;
                            break;
                        }
                    }
                }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { RealDStarLite, RealRRTStar, RealACO, RealHCFA };
}
